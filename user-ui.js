...
function processTodaysTransactions() {
    ...
    const todaysTransactions = allTransactions.filter(tx => tx.date.startsWith(today));
...
function displayNotifications() {
    ...
    list.innerHTML = '';
    const today = new Date().toISOString().split('T')[0];
    const todaysTransactions = allTransactions.filter(tx => tx.date.startsWith(today)).sort((a,b) => new Date(b.date) - new Date(a.date));
